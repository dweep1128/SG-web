-- Run this now. It completes a database where the first SQL query stopped after indexes.
-- It is safe to run once after `schema.sql`.
...
drop policy if exists "public reads models" on public.scooter_models;
drop policy if exists "public reads active products" on public.products;
drop policy if exists "public reads product images" on public.product_images;
drop policy if exists "public reads compatibility" on public.product_compatibility;
drop policy if exists "users read own profile" on public.profiles;
drop policy if exists "users update own profile" on public.profiles;
drop policy if exists "staff manage categories" on public.categories;
drop policy if exists "staff manage models" on public.scooter_models;
drop policy if exists "staff manage products" on public.products;
drop policy if exists "staff manage product images" on public.product_images;
drop policy if exists "staff manage compatibility" on public.product_compatibility;
...
where not exists (select 1 from public.product_images image where image.product_id=p.id and image.image_url=d.image_url);
