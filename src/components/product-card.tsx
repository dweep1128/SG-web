...
export function ProductCard({ product }: { product: Product }) {
  ...
  return <article className="product-card"><Link className="product-img" href={`/products/${product.slug}`}><span className={`stock ${product.stock === "limited" ? "limited" : product.stock === "out_of_stock" ? "out" : ""}`}>{stockLabel(product.stock)}</span>{image ? <img src={image.image_url} alt={image.alt_text || product.name}/> : <span className="product-placeholder"><b>SK</b><small>PRODUCT IMAGE<br/>COMING SOON</small></span>}</Link><p className="sku">{product.sku}</p><Link href={`/products/${product.slug}`}><h3>{product.name}</h3></Link><p className="meta">{product.voltage || "Specification on request"} · MOQ {product.moq}</p>{fits && <p className="fits">Fits: {fits}</p>}<button className="add-btn" disabled={product.stock === "out_of_stock"} onClick={() => add({ product_id: product.id, name: product.name, sku: product.sku })}>{product.stock === "out_of_stock" ? "Enquire for availability" : "Add to quote +"}</button></article>;
}
