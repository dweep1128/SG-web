...
function Header({ openCart }: { openCart: () => void }) {
  ...
  return <><div className="topbar"><div className="shell"><span>WHOLESALE EV SPARE PARTS · PAN-INDIA DISPATCH</span><a href="#contact">Need help? Talk to an expert ↗</a></div></div><header className="site-header"><div className="shell nav-row"><Link className="brand" href="/"><span className="brand-mark"><i/><i/><i/></span><span>SK <small>TRADERS</small></span></Link><nav className="desktop-nav"><Link href="/">Home</Link><Link href="/products">Parts</Link><Link href="/#find-part">Find my part</Link><Link href="/#wholesale">Wholesale</Link></nav><div className="nav-actions"><ThemeButton/><button className="cart-btn" onClick={openCart} aria-label="Open quote list">▢ Quote list <b>{count}</b></button><button className="nav-menu" onClick={() => setMenu(!menu)} aria-label="Open navigation">☰</button></div></div>{menu && <nav className="mobile-menu shell open"><Link onClick={() => setMenu(false)} href="/">Home</Link><Link onClick={() => setMenu(false)} href="/products">Parts</Link><Link onClick={() => setMenu(false)} href="/#find-part">Find my part</Link><Link onClick={() => setMenu(false)} href="/#wholesale">Wholesale</Link><Link onClick={() => setMenu(false)} href="/account/login">Login</Link></nav>}</header></>;
}
...
function CartDrawer({ open, close, items, setItems }: { open: boolean; close: () => void; items: CartItem[]; setItems: (items: CartItem[]) => void }) {
  ...
  return <aside className={`cart-drawer ${open ? "open" : ""}`} aria-hidden={!open}><div className="cart-head"><div><span className="eyebrow">YOUR SELECTION</span><h2>Quote list</h2></div><button className="cart-close" onClick={close}>×</button></div><div className="cart-items">{items.length ? items.map(item => <div className="cart-line" key={item.product_id}><div><b>{item.name}</b><small>{item.sku}</small></div><div className="quantity"><button onClick={() => update(item.product_id, -1)}>−</button><output>{item.quantity}</output><button onClick={() => update(item.product_id, 1)}>+</button></div></div>) : <p className="empty">Your quote list is empty.<br/>Add products to request a bulk quote.</p>}</div>{items.length > 0 && <form className="quote-form" onSubmit={sendQuote}><div className="compact-grid"><input required name="full_name" placeholder="Your name *"/><input required name="phone" placeholder="Phone / WhatsApp *"/></div><div className="compact-grid"><input name="business_name" placeholder="Business name"/><input name="city" placeholder="City"/></div><input name="gst_number" placeholder="GST number (optional)"/><textarea name="message" placeholder="Anything else we should know?"/><button className="btn btn-primary" disabled={sending} type="submit">{sending ? "Sending…" : "Request quote on WhatsApp ↗"}</button>{message && <p className={message.includes("saved") ? "form-success" : "form-error"}>{message}</p>}</form>}</aside>;
}
